 ...
 *      **Criterion (4) is the only thing separating a real-but-never-resumed
 *      session from a ghost**, because a session recorded by 0.1.7's
 *      SessionStart has no preview, no fingerprint and no ai_title either.
 *      That makes the disk scan safety-critical rather than merely an
 *      optimisation — see `assessScanTrust` below.
 ...
 *      yet.
 *   6. No operator intent attached: no name on ANY channel, no
 *      parent_session_id, not referenced as a parent by another record, no
 *      linked tasks/projects, and `outcome === 'open'` (a closed session was
 *      deliberately closed).
 *
 *      "Any channel" rather than "no alias" because the channel set is open
 *      (see lib/names.mjs). Enumerating the channels here would put this
 *      criterion one release behind every new namer, and being behind means
 *      deleting a record whose only name arrived through a channel this file
 *      had not heard of — which is what happened to `cc_custom_title`, the
 *      surface the name model itself calls the strongest statement of intent
 *      any namer produces. A cleared entry counts too: somebody named the
 *      session and then unnamed it, and both halves of that are a person
...
import { hasAnyName } from './names.mjs';
...
export function assessScanTrust(scan) {
  ...
  // Zero files is not "a machine with no transcripts" in any realistic
  // deployment — the records we are being asked to prune were written by
  // sessions whose transcripts live under that very root. Treat it as a
  // misconfigured root, and make the operator say otherwise out loud.
...
export function computePruneCandidates(projection, opts = {}) {
  ...
  for (const [stableId, s] of Object.entries(sessions)) {
    ...
    // Every criterion below is about Claude Code's artefacts: a preview and
    // fingerprints derived from Claude prompts, and a transcript located by
    // `claude_session_id`. A codex record satisfies all of them vacuously —
    // it has no claude ids, and a rollout whose opening turns were all
    // injected blocks leaves no preview either — so without this guard the
    // ghost sweep would delete real sessions whose transcript is sitting on
    // disk in a directory this scan never looks at.
    if ((s.source ?? 'claude') !== 'claude') { spare('not_a_claude_session'); continue; }
    ...
    // Ordered after the two legacy mirrors so the `--json` tally keeps saying
    // `has_ai_title` / `has_alias` for the cases it always said them for; this
    // key only ever accounts for a record the old checks would have missed.
    if (hasAnyName(s)) { spare('has_name'); continue; }
...
export async function runPrune(opts = {}) {
  ...
  // Delete path only. An untrusted scan means criterion (4) proved nothing,
  // so the candidate list is not a ghost list — it is "every session that was
  // never resumed". Refuse before taking the lock; the operator can re-run
  // with the escape hatch once they have looked at `disk_scan.root`.
...
/**
 * Human-readable refusal text. Names the root that was scanned, because the
 * realistic causes (sudo, cron, container, a typo'd env var) all show up as
 * "that is not the root I expected" the moment the operator reads it.
 *
 * @param {{ dirCount?: number, fileCount?: number, errors?: string[],
 *   root?: string }} scan
 * @param {{ reasons: string[] }} trust
 * @returns {string}
 */
function describeUntrustedScan(scan, trust) {
  ...
  return (
    `refusing to prune: transcript scan is not trustworthy — ${parts.join(' · ')}. ` +
    '"No transcript on disk" is the only criterion separating a real session ' +
    'that was never resumed from a ghost, so an empty or failed scan would ' +
    'make every never-resumed session look prunable. Check ' +
    'DRUUMEN_CLAUDE_PROJECTS_ROOT / HOME (sudo, cron and containers change ' +
    'it), then re-run. Override with --accept-untrusted-scan only if you are ' +
    'certain this machine really has no transcripts.'
  );
}
