...
export function currentNameValue(session, channel) {
  ...
  if (derived && typeof session?.[derived] === 'string' && session[derived].length > 0) {
    ...
    // Empty means the mirror held nothing a terminal may print, which is the
    // same answer as "this channel was never set" — not `''`, which would
    // read as "it is called the empty string".
...
export function nameValuesFromSession(session) {
  ...
  //
  // Sanitised here too, and NOT as a formality: this branch is the one path
  // by which an uncleaned value still reaches a terminal. The entries above
  // came through `nameChangeFromEvent`, which sanitises — so a value that is
  // ENTIRELY escape bytes (`\x1b[31m\x1b[0m`) cleans to empty, fails
  // `isValidNameValue`, and never becomes a `names[]` entry at all. The
  // channel then looks unset, this fallback picks the raw mirror up, and the
  // display chain prints the original bytes. Partial junk was already safe
  // (the cleaned entry exists and wins on the line above); it is precisely
  // the "cleans to nothing" case that leaked, and it leaked into the label
  // `find` / `search` print unescaped.
  //
  // Empty after cleaning means the channel has no usable name, so it is left
  // undefined rather than set to `''` — the chain must fall through to the
  // next channel, not stop on a name that renders as nothing.
  for (const [channel, field] of Object.entries(DERIVED_FIELD_BY_CHANNEL)) {
    if (values[channel] !== undefined) continue;
