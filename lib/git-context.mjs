 ...
 */
export async function gitContextFast(opts = {}) {
  const cwd = typeof opts.cwd === 'string' && opts.cwd.length > 0 ? opts.cwd : process.cwd();
  const totalBudgetMs = Number.isFinite(opts.totalBudgetMs) && opts.totalBudgetMs > 0
    ? opts.totalBudgetMs
    : DEFAULT_TOTAL_BUDGET_MS;
  ...
  const probe = await runGit(
    ...
    { cwd, deadlineAt: Date.now() + totalBudgetMs },
    ctx,
  );
  ...
  if (!probe.ok) {
    if (probe.spawnFailed || probe.timedOut) {
      ...
      // Ran fine, exit != 0 => outside a repo. The diagnostic runGit pushed is
      // noise for this expected case (same treatment as gitContext probe 1).
      if (ctx.errors.length > 0) ctx.errors.pop();
      ctx.status = 'not_a_repo';
    }
    return ctx;
  ...
  if (lines[0] && lines[0].startsWith('/')) ctx.worktreePath = lines[0];
  if (lines[1] && /^[0-9a-f]{40}$/i.test(lines[1])) ctx.head = lines[1].toLowerCase();
  // `--abbrev-ref HEAD` prints the literal string "HEAD" on a detached HEAD;
  // map that to null so consumers never store "HEAD" as a branch name.
  if (lines[2] && lines[2] !== 'HEAD') ctx.branch = lines[2];
