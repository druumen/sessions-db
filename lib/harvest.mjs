...
export async function harvestFromTranscript({ stableId, transcriptPath, recordTargetOpts, dryRun = false }) {
  ...
  // list. The names half above had to hand-write its comparison and the
  ...
  for (const link of titles.prLinks ?? []) {
    ...
    planned.push(event);
    // The probe already produced the post-write list; keeping it also in the
    // dry-run path is what makes a dry run over a session with two new links
    // report two, instead of reporting the first one twice.
    if (session) session.pr_links = probe.pr_links;
    if (dryRun) continue;

    try {
      await tryUpdateProjection(event, { ...recordTargetOpts, lockTimeoutMs: 1500 });
      // Keep the local copy in step so a second link in the same batch is
      // compared against the state this loop has already produced, not
      // against the stale load. Without this, two NEW links in one scan
      // would both compare clean and the second could re-append on the next
      // prompt as well.
      if (session) session.pr_links = probe.pr_links;
    } catch {
...
export async function runHarvest(opts = {}) {
  ...
  for (const [stableId, session] of Object.entries(sessions)) {
    if (budget <= 0) break;
    if (!opts.all && !needsHarvest(session)) continue;
    out.scanned += 1;
...
/** A record the backfill is for: one of the two halves was never harvested. */
function needsHarvest(session) {
  const names = Array.isArray(session.names) ? session.names : [];
  const hasHarvestedName = names.some((n) => n && n.channel !== 'alias' && n.value);
  const hasLinks = Array.isArray(session.pr_links) && session.pr_links.length > 0;
  return !hasHarvestedName || !hasLinks;
}
