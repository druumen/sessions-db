 ...
 */
export function distinctCurrentNames(session) {
...
export function recordText(record) {
  if (!record) return '';
  // Codex rollout shape. A different product's transcript, so a different
  // record layout: conversation turns are `response_item` envelopes whose
  // payload carries `role` and a `content[]` of `{text}` parts. Handled here
  // rather than in a second scanner because everything downstream — snippet
  // extraction, the size budget, the cache — is identical once the text is
  // out, and two scanners would be two places to forget a shape.
