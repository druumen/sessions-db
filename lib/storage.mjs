 ...
 *
 * So the fold is used as a source of corrected name blocks only. A session
 * the log knows about takes the log's answer — the log is the SSoT for names
 * and its `names[]` is complete. A session the log has nothing to say about
...
function backfillNamesModel(parsed, opts) {
  ...
  for (const [stableId, session] of Object.entries(parsed.sessions)) {
    ...
    session.names = fresh.names;
    session.display_name = fresh.display_name;
    session.display_name_channel = fresh.display_name_channel;
  }
