...
import { NAMES_MODEL_VERSION } from './names.mjs';
import {
  applyEvent,
  emptyProjection,
  mergeNamesFromLog,
...
/**
 * Resolve the storage DIRECTORY for a given opts shape (the directory that
 * holds events.jsonl / sessions-db.json / the lockfile).
 *
 * Exists so siblings of the canonical triple — currently the pending area
 * (`lib/pending.mjs`) — can live next to the rest of the state without
 * re-deriving the four-shape opts resolution. Deriving it from the projection
 * path rather than re-running the priority chain guarantees the pending area
 * always lands beside the projection it is a staging buffer for, including
 * under the legacy `opts.paths` test shape where the three files may be
 * pinned to arbitrary locations.
 *
 * @param {{ paths?: typeof PATHS, rootPath?: string, root?: string, cwd?: string }} [opts]
 * @returns {string} absolute directory path
 */
export function storageDir(opts = {}) {
  return dirname(resolvePaths(opts).projectionPath);
}

/**
 * Resolve the projection lockfile path for a given opts shape.
 *
 * Exported for the one caller that legitimately needs to hold the projection
 * lock across a MULTI-event transaction (`lib/prune.mjs`), which cannot go
 * through `tryUpdateProjection`'s one-event-per-lock-cycle shape. Exposing
 * the resolver — rather than letting that caller re-derive the path — is what
 * guarantees both writers contend on the SAME file; a drifted path would look
 * like it works while silently providing no mutual exclusion at all.
 *
 * @param {{ paths?: typeof PATHS, rootPath?: string, root?: string, cwd?: string }} [opts]
 * @returns {string} absolute path to the lockfile
 */
export function lockPathFor(opts = {}) {
...
/**
 * Recompute `names[]` from the event log for a cache written before the
 * current name model, then stamp it so this only ever happens once.
 *
 * ## Why the cache cannot be repaired where it stands
 *
 * `names[].set_count` and `names[].observed_from` are folds of the log, not
 * fields any single event carries. When a new event lands on a session whose
 * `names[]` is empty because it predates the model, the reducer materialises
 * that channel from scratch and records one set — for a session the log may
 * say was renamed three times. Nothing ever corrects it afterwards: the
 * derived-name refresh only runs on sessions that receive an event, so on a
 * real database a single write materialises exactly one session's names and
 * every other session keeps the wrong answer indefinitely. Measured on the
 * 632-session reference database, one alias write materialised 1 record.
 *
 * ## Why it overlays rather than replaces
 *
 * The obvious repair — discard the cache, return a full rebuild — assumes
 * events.jsonl still contains everything the cache was built from. That is
 * not guaranteed: logs get retired and split, and a caller that plants a
 * projection without a log is a legitimate shape. Replacing wholesale would
 * turn "a counter is stale" into "every session is gone".
 *
 * So the fold is used as a source of corrected name blocks only, and it is
 * applied at CHANNEL granularity (`mergeNamesFromLog`) rather than by
 * overwriting the session's whole `names[]`. The session-level version of
 * this was overlaid for exactly the reason above and then threw the same
 * thing away one level down: a rotated log can still speak for a session
 * through one channel while the cache holds channels it no longer mentions,
 * and replacing the block deleted those. Per channel the repair needs no
 * assumption about the log being complete — see `mergeNamesFromLog` for what
 * the remaining residual is (a `set_count`, never a name) and for why the
 * legacy `alias` / `ai_title` mirrors are re-derived in the same pass.
 *
 * A session the log has nothing to say about is left exactly as it was.
 *
 * Cost: one fold, once per database (18 ms over the 2018-event reference log,
 * against the 1500 ms lock budget the hook allows itself). The stamp goes on
 * whatever the caller next saves, and every later load takes the fast path.
 *
 * If the fold throws (mid-file corruption in events.jsonl escalates), the
 * cache is returned untouched and unstamped: an uncorrected counter is a far
 * smaller problem than a failed load, and leaving the stamp off means the
 * repair is retried once the log is readable again.
 *
 * @param {object} parsed projection as read from disk (mutated in place)
 * @param {{ paths?: typeof PATHS, root?: string }} opts
 * @returns {object} the same projection reference
 */
