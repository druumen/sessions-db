...
var MAX_PR_NUMBER = 9999999;
var MAX_PR_NUMBER_DIGITS = String(MAX_PR_NUMBER).length;
...
var BARE_NUMBER_RE = new RegExp(`^[!#]?(\\d{1,${MAX_PR_NUMBER_DIGITS}})$`);
...
function applyEvent(projection, event) {
  ...
  if (!OBSERVATION_ONLY_OPS.has(op) && op !== "sweep" && op !== "session_prune" && ts && (!session.last_progress_at || ts > session.last_progress_at)) {
...
var OBSERVATION_ONLY_OPS = /* @__PURE__ */ new Set(["name_set", "pr_link_seen", "ai_title_seen"]);
...
function reduceCodexSessionSeen(session, event) {
  ...
  const id = typeof p.codex_session_id === "string" && p.codex_session_id.length > 0 ? p.codex_session_id : null;
  ...
  if (typeof p.first_prompt_preview === "string" && p.first_prompt_preview.length > 0 && !session.first_prompt_preview) {
    session.first_prompt_preview = p.first_prompt_preview;
  }
  if (typeof p.started_at === "string" && p.started_at.length > 0 && (!session.created_at || p.started_at < session.created_at)) {
