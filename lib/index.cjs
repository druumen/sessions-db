...
function isValidNameValue(value) {
  ...
  return typeof value === "string" && value.length > 0 && value.length <= MAX_NAME_VALUE_LEN;
}
...
function nameChangeFromEvent(event) {
  ...
  const observedAt = typeof p.observed_at === "string" && p.observed_at.length > 0 ? p.observed_at : null;
  return {
    channel,
    value,
    source,
...
function nameValuesFromSession(session) {
  ...
  for (const [channel, field] of Object.entries(DERIVED_FIELD_BY_CHANNEL)) {
    if (values[channel] === void 0 && typeof session[field] === "string" && session[field].length > 0) {
...
function extractLatestTitles(path, opts = {}) {
  ...
  try {
    ...
    const out = { aiTitle: null, customTitle: null, agentName: null, sessionId: null };
    const FIELDS = {
    ...
    for (let i = lines.length - 1; i >= 0; i--) {
      ...
      if (out.aiTitle !== null && out.customTitle !== null && out.agentName !== null) break;
    }
    return out;
  } finally {
    try {
