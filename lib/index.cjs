...
function emptySession(stableId, ts) {
  return {
    ...
    // `{"type":"ai-title", "aiTitle": "..."}` records. Distinct from `alias`
    // (which is a deliberate human label set via `sessions-db alias`).
    // Display priority for find: alias ?? ai_title ?? first_prompt_preview.
    ai_title: null,
    claude_session_ids: [],
    transcript_files: [],
    fingerprints: {
...
function isPromoterAlive(opts = {}, checkOpts = {}) {
  ...
  try {
    const st = (0, import_node_fs8.statSync)((0, import_node_path7.join)(pendingDir(opts), PROMOTER_MARKER));
