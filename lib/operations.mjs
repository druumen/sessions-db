...
export async function setAlias(opts) {
  ...
  if (hasAlias && alias.length > MAX_NAME_VALUE_LEN) {
    return {
      ok: false,
  ...
  // Writes `name_set`, not the historical `alias_set`. The old op is still
  // understood on read forever (an append-only log cannot be rewritten), but
  // there is no reason to keep minting the special case now that a general
  // one exists — and one write path means the alias cannot end up recorded
  // differently from every other name.
  return commitOp({
    op: 'name_set',
    stableId,
    payload: aliasSetPayload(wantsClear ? null : alias),
    opts,
  });
}
...
export function aliasSetPayload(value) {
  return nameSetPayload({ channel: CHANNEL_ALIAS, value, source: SOURCE_HUMAN });
}
...
export async function setName(opts) {
  ...
  const nextValue = wantsClear ? null : value;
  if (!isValidNameValue(nextValue)) {
    return {
      ok: false,
      error: `setName: value must be a non-empty string of at most ${MAX_NAME_VALUE_LEN} characters, or null`,
    };
  }
  const nextSource = source ?? SOURCE_HUMAN;
