 ...
 *      want the SSoT to grow `name_set` events for non-existent sessions.
 *
...
export async function setAlias(opts) {
  ...
  // Sanitised before the length check so the cap applies to what is actually
  // stored, and so an alias that is nothing but escape sequences fails here
  // with a clear message instead of being written as an empty name.
  const cleanAlias = hasAlias ? sanitizeNameValue(alias) : null;
  if (hasAlias && cleanAlias.length === 0) {
    return { ok: false, error: 'setAlias: alias must contain printable characters' };
  }
  if (hasAlias && cleanAlias.length > MAX_NAME_VALUE_LEN) {
    return {
      ok: false,
      error: `setAlias: alias must be at most ${MAX_NAME_VALUE_LEN} characters`,
    };
  }
  const exists = await ensureSessionExists(stableId, opts);
  if (!exists.ok) return { ok: false, error: exists.error };
  // Keeps minting `alias_set`, deliberately, even though `name_set` is the
  // general form every other channel uses.
  //
  // The reducer already runs `applyNameFromEvent` on `alias_set`, so this op
  // lands in the channel model exactly as `name_set` would — nothing about the
  // name model needs the new op. What the old op additionally buys is that a
  // pre-0.3.0 reducer still understands it. That matters more here than
  // anywhere else: `loadProjection` rebuilds from the log whenever the cache
  // is missing or corrupt, so an older reader on the same machine drops the
  // alias without anybody asking it to rebuild, and `alias` is the one channel
  // a human sets by hand. Breaking backward compatibility on the channel that
  // carries the most intent, for no gain, is not a trade worth making.
  return commitOp({
    op: 'alias_set',
    stableId,
 ...
 */
export function aliasSetPayload(value) {
 ...
 * @returns {Promise<{ ok: boolean, event_id?: string, error?: string }>}
 */
export async function setName(opts) {
  if (!opts || typeof opts !== 'object') {
  ...
  if (!isValidChannel(channel)) {
    return {
      ok: false,
      error:
        'setName: channel must be 1-64 chars of [A-Za-z0-9._-] starting alphanumeric',
    };
  }
  ...
  // Defaults to `harvest`, matching `nameSetPayload` and
  // `nameChangeFromEvent`. The three used to disagree, and `human` was the
  // wrong one to pick: the dimension exists so a consumer can ask "which
  // names did a PERSON give this session", and a caller that did not say who
  // authored a name is, by construction, reporting something it observed.
  // Claiming human authorship on their behalf poisons the only question the
  // axis answers. `setAlias` passes `human` explicitly, as does any caller
  // that actually knows.
  ...
  // `observedAt` is typed `Iso8601`. Silently storing "not-a-date" in it and
  // returning ok made the declared type a lie all the way down to whatever
  // sorts the history view; refusing is the only answer that keeps the type
  // meaningful, and the caller can always omit the field to get the event ts.
  if (observedAt !== undefined && observedAt !== null && !isIso8601(observedAt)) {
    return {
      ok: false,
      error: 'setName: observedAt must be an ISO 8601 timestamp (e.g. 2026-08-19T10:00:00.000Z)',
    };
  }
  if (observedFrom !== undefined && observedFrom !== null &&
      normalizeObservedFrom(observedFrom) === null) {
    return {
      ok: false,
