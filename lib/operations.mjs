...
/**
 * Resolve `{ root, paths }` opts into a single object suitable for storage
 * primitives. Both fields are optional; when omitted we let storage fall
 * back to its defaults (cwd + canonical PATHS).
 */
function storageOpts({ root, paths } = {}) {
  const out = {};
